...
abstract class FFcommon {
	...
	protected FFcommon(@Nonnull String path,
			@Nonnull ProcessFunction runFunction) {
		...
		try
		{
			...
			if (ProcessUtils.waitForWithTimeout(p, 1, TimeUnit.SECONDS) != 0)
			{
				...
				throw new IOException(
						path + " returned non-zero exit status. Check stdout.");
			}
		} catch (TimeoutException e)
		{
			throw new IOException(
					"Timed out waiting for " + path + " to finish.");
		}
	}
	 ...
	 * @throws IOException
	 *                         If there is an error capturing output from the
	 *                         binary.
	 */
	public synchronized @Nonnull String version() throws IOException
	{
		if (this.version == null)
		{
			...
			try
			{
				...
				CharStreams.copy(r, CharStreams.nullWriter()); // Throw away
																// rest of the
																// output
	...
	public void run(List<String> args) throws IOException
	{
		...
		try
		{
			// TODO Move the copy onto a thread, so that FFmpegProgressListener
			// can be on this thread.

			// Now block reading ffmpeg's stdout. We are effectively throwing
			// away the output.
			CharStreams.copy(wrapInReader(process), System.out); // TODO Should
																	// I be
																	// outputting
																	// to
																	// stdout?
